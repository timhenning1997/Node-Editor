...
class Edge(Serializable):
    ...
    def __init__(self, scene:'Scene', start_socket:'Socket'=None, end_socket:'Socket'=None, edge_type=EDGE_TYPE_DIRECT):
        ...
        self.grEdge.setLockedStatus(self.locked)
    ...
    def updatePositions(self):
        ...
        angle = self.start_socket.node.grNode.rotation()
        newX = source_pos[0] * math.cos(math.radians(angle)) - source_pos[1] * math.sin(math.radians(angle))
        newY = source_pos[0] * math.sin(math.radians(angle)) + source_pos[1] * math.cos(math.radians(angle))
        source_pos[0] = newX + self.start_socket.node.grNode.pos().x()
        source_pos[1] = newY + self.start_socket.node.grNode.pos().y()

        self.grEdge.setSource(*source_pos)
        if self.end_socket is not None:
            end_pos = self.end_socket.getSocketPosition()
            angle = self.end_socket.node.grNode.rotation()
            newX = end_pos[0] * math.cos(math.radians(angle)) - end_pos[1] * math.sin(math.radians(angle))
            newY = end_pos[0] * math.sin(math.radians(angle)) + end_pos[1] * math.cos(math.radians(angle))
            end_pos[0] = newX + self.end_socket.node.grNode.pos().x()
            end_pos[1] = newY + self.end_socket.node.grNode.pos().y()

            self.grEdge.setDestination(*end_pos)
        else:
            if self.grEdge.posDestination == [200, 100]:
            ...
            for socket in old_sockets:
                if socket and socket.node:
                    ...
                    if socket.is_input:
                        ...
                        socket.node.onOutputChanged(socket)
    ...
    def serialize(self) -> OrderedDict:
        return OrderedDict([
            ...
            ('edge_visibility', self.grEdge.hiddenStatus),
        ])
    ...
    def deserialize(self, data:dict, hashmap:dict={}, restore_id:bool=True, *args, **kwargs) -> bool:
        ...
        self.grEdge.setLockedStatus(data['locked_status'])
        self.grEdge.hiddenStatus = data['edge_visibility']
