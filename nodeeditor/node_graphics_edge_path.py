...
class GraphicsEdgePathBezier(GraphicsEdgePathBase):
    ...
    def calcPath(self) -> QPainterPath:
        ...
        dist = (abs(d[0] - s[0]) + abs(d[1] - s[1])) * 0.25
        ...
        maxDist = min(abs(dist * 1.5), 200)
        sourceControlPointX = s[0]
        sourceControlPointY = s[1]
        destinationControlPointX = d[0]
        destinationControlPointY = d[1]

        if self.owner.edge.start_socket is not None:
            rotStart = self.owner.edge.start_socket.node.grNode.rotation()
            if self.owner.edge.end_socket is not None:
                rotDest = self.owner.edge.end_socket.node.grNode.rotation()
            else:
                rotDest = 0

            if self.owner.edge.start_socket.is_input:

                maxDistX = maxDist * math.cos(math.radians(rotStart))
                maxDistY = maxDist * math.sin(math.radians(rotStart))

                sourceControlPointX = s[0] - maxDistX
                sourceControlPointY = s[1] - maxDistY

                maxDistX = maxDist * math.cos(math.radians(rotDest))
                maxDistY = maxDist * math.sin(math.radians(rotDest))

                destinationControlPointX = d[0] + maxDistX
                destinationControlPointY = d[1] + maxDistY
            else:
                maxDistX = maxDist * math.cos(math.radians(rotStart))
                maxDistY = maxDist * math.sin(math.radians(rotStart))
        ...
        path.cubicTo(sourceControlPointX, sourceControlPointY, destinationControlPointX, destinationControlPointY, d[0], d[1])
