...
from nodeeditor.var_type_conf import TYPE_COLORS, EDGE_COLOR, EVAL_HIGHLIGHT_COLOR
...
class Scene(Serializable):
    ...
    def initGlobalColors(self):
        self.socketColors = TYPE_COLORS.copy()
        self.edgeColor = EDGE_COLOR
        self.isGetEdgeColorFromSocket = False
        self.updateSocketColors()
        self.evalHighlightColor = EVAL_HIGHLIGHT_COLOR

    def updateSocketColors(self):
        for node in self.nodes:
            for socket in (node.inputs+node.outputs):
                if socket:
                    socket.grSocket.updateColor()

    def updateEdgeColors(self):
        for node in self.nodes:
            for socket in (node.inputs+node.outputs):
                for edge in socket.edges:
                    if edge:
                        edge.grEdge.changeColor(self.edgeColor)

    def updateEvalHighlightColors(self):
        for node in self.nodes:
            node.grNode.setEvaluatedAnimationColor(self.evalHighlightColor)
    ...
    def saveToFileAsNodeGroup(self, filename: str):
        """
        Save all nodes to the file on disk.

        :param filename: where to save this scene
        :type filename: ``str``
        """
        with open(filename, "w") as file:
            file.write( json.dumps( self.serializeAsNodeGroup(), indent=4 ) )
            print("saving nodeGroup to", filename, "was successfull.")
    ...
    def getNodeClassFromData(self, data: dict) -> 'Node class instance':
        ...
        """
        nodeClass = None
        if data["_path"] != "" and data["_filename"] != "":
            try:
                nodeClass = self.getView().parent().getNodeClassFromPath(data["_path"], data["_filename"])
    ...
    def serialize(self) -> OrderedDict:
        ...
        socketColors = []
        socketColorsAlpha = []
        for color in self.socketColors:
            socketColors.append(color.name())
            socketColorsAlpha.append(color.alpha())

        return OrderedDict([
            ('id', self.id),
            ('socket_colors', socketColors),
            ('socket_colors_alpha', socketColorsAlpha),
            ('edge_color', self.edgeColor.name()),
            ('edge_color_alpha', self.edgeColor.alpha()),
            ('is_get_edge_color_from_socket', self.isGetEdgeColorFromSocket),
            ('eval_highlight_color', self.evalHighlightColor.name()),
            ('eval_highlight_color_alpha', self.evalHighlightColor.alpha()),
    ...
    def serializeAsNodeGroup(self) -> OrderedDict:
        nodes, edges = [], []
        for node in self.nodes: nodes.append(node.serialize())
        for edge in self.edges: edges.append(edge.serialize())
    ...
    def deserialize(self, data: dict, hashmap: dict={}, restore_id: bool=True, *args, **kwargs) -> bool:
        ...
        try:
            self.evalHighlightColor = QColor(data['eval_highlight_color'])
            self.evalHighlightColor.setAlpha(data['eval_highlight_color_alpha'])
            self.updateEvalHighlightColors()
        except Exception as e: dumpException(e)


        # update sockets colors
        count = 0
        try:
            for color in data['socket_colors']:
                self.socketColors[count] = QColor(color)
                self.socketColors[count].setAlpha(data['socket_colors_alpha'][count])
                count += 1
            self.updateSocketColors()
        except Exception as e:
            dumpException(e)
        ...
        # update edges colors
        try:
            self.edgeColor = QColor(data['edge_color'])
            self.edgeColor.setAlpha(data['edge_color_alpha'])
            self.isGetEdgeColorFromSocket = data['is_get_edge_color_from_socket']
            self.updateEdgeColors()
        except Exception as e: dumpException(e)

        for node in self.nodes:
            node.evalAfterDeserialize()
