...
class QDMGraphicsEdge(QGraphicsPathItem):
    ...
    def __init__(self, edge:'Edge', parent:QWidget=None):
        ...
        self.isGetColorFromSocket = self.edge.scene.isGetEdgeColorFromSocket
    ...
    def initAssets(self):
        ...
        self._color = self._default_color = self.edge.scene.edgeColor
        self._color_dragging = QColor("#101010")
        self._color_selected = QColor("#00ff00")
        self._color_hovered = QColor("#FF37A6FF")
        self._color_hidden = QColor("#00000000")
        self._pen = QPen(self._color)
        self._pen_selected = QPen(self._color_selected)
        self._pen_dragging = QPen(self._color_dragging)
        self._pen_hovered = QPen(self._color_hovered)
        self._pen_hidden = QPen(self._color_hidden)
        self._pen_dragging.setStyle(Qt.DashLine)
        self._pen.setWidthF(3.0)
        self._pen_selected.setWidthF(3.0)
        self._pen_dragging.setWidthF(3.0)
        self._pen_hovered.setWidthF(5.0)
        self._pen_hidden.setWidthF(3.0)
    ...
    def setColorFromSockets(self) -> bool:
        ...
        try:
            socket_type_start = self.edge.start_socket
            socket_type_end = self.edge.end_socket
            if socket_type_start.is_input:
                self.changeColor(self.edge.start_socket.grSocket.getSocketColor(socket_type_end.socket_type))
            else:
                self.changeColor(self.edge.start_socket.grSocket.getSocketColor(socket_type_start.socket_type))
        except: pass
    ...
    def lockEdge(self):
        self.edge.locked = True
        self.setFlag(QGraphicsItem.ItemIsSelectable, False)

    def editEdge(self):
        self.edge.locked = False
    ...
    def paint(self, painter, QStyleOptionGraphicsItem, widget=None):
        ...
        if self.isGetColorFromSocket:
            self.setColorFromSockets()

        painter.setBrush(Qt.NoBrush)

        if self.hovered and self.edge.end_socket is not None and not self.edge.locked:
            painter.setPen(self._pen_hovered)
            painter.drawPath(self.path())

        if self.edge.end_socket is None:
            painter.setPen(self._pen_dragging)
        elif not self.hiddenStatus:
            painter.setPen(self._pen if not self.isSelected() else self._pen_selected)
        else:
            painter.setPen(self._pen_hidden if not self.isSelected() else self._pen_selected)
