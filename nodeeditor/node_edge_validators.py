...
def edge_cannot_connect_input_and_output_of_different_type(input: 'Socket', output: 'Socket') -> bool:
    ...
    if input.socket_type == VAR_TYPE_NOT_DEFINED or output.socket_type == VAR_TYPE_NOT_DEFINED:
        return True
...
def edge_cannot_connect_input_and_output_of_different_supported_type(input: 'Socket', output: 'Socket') -> bool:
    """Edge is invalid if it connects sockets with different supported types"""

    if any(x in input.supportedTypes for x in output.supportedTypes):
        return True

    if VAR_TYPE_NOT_DEFINED in (output.supportedTypes + input.supportedTypes):
        return True

    print_error("Connecting sockets with different supported types")
    return False
