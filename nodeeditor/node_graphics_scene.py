...
from PyQt5.QtCore import pyqtSignal, QRect, QLine, Qt, QEvent, QPoint
from PyQt5.QtGui import QColor, QPen, QFont, QPainter, QContextMenuEvent
...
class QDMGraphicsScene(QGraphicsScene):
    ...
    def __init__(self, scene: 'Scene', parent: QWidget=None):
        ...
        self.isSnappingToGrid = False
        self.isSnappingToGridSquares = False
    ...
    def drawBackground(self, painter:QPainter, rect:QRect):
        ...
        if self.scene.getView().zoom in [12]:
            self.gridSize = 5
            self._pen_light.setWidthF(0.4)
            self._pen_dark.setWidthF(0.4)
        elif self.scene.getView().zoom in [11, 5]:
            self.gridSize = 20
            self._pen_light.setWidth(1)
            self._pen_dark.setWidth(1)
        elif self.scene.getView().zoom in [4, -2]:
            self.gridSize = 100
            self._pen_light.setWidth(4)
            self._pen_dark.setWidth(4)
        elif self.scene.getView().zoom in [-3]:
            self.gridSize = 500
            self._pen_light.setWidth(20)
            self._pen_dark.setWidth(20)
