...
class EdgeRerouting:
    ...
    def stopRerouting(self, target: 'Socket'=None):
        ...
        for affected_node, edge in affected_nodes:
            ...
            elif edge.start_socket in affected_node.outputs:
                affected_node.onOutputChanged(edge.start_socket)
            if edge.end_socket in affected_node.inputs:
                affected_node.onInputChanged(edge.end_socket)
            elif edge.end_socket in affected_node.outputs:
                affected_node.onOutputChanged(edge.end_socket)
